...
class BookingFrame(BaseFrame):
   def __init__(self, parent, app, system):
       ...
       self.user_var.set(f"Booking for: {self.app.current_user}") # Initialize user label
       tk.Label(self, textvariable=self.user_var).pack() # Show logged-in user
   ...
   def handle_booking(self):
       ...
       if flight:
           ...
           messagebox.showerror("Error", "Invalid flight code.")
   ...
   def update_user_label(self):
       self.user_var.set(f"Booking for: {self.app.current_user}") # Update the user label
